...
class DAVE(nn.Module):
    ...
    #def forward(self, v_equi, v_cube, a,  aem, eq_b):
        ...
        print("xV1_equi shape:", xV1_equi.shape)  # ([1, 512, 1, 8, 10])
        #  如果修改完模型的输入之后变成了这个 xV1_equi shape: torch.Size([1, 512, 1, 10, 20])
        xV1_equi = F.interpolate(xV1_equi, size=(1, 8, 10), mode='trilinear', align_corners=True) # 这一行完全是为了让模型跑通，看loss而刻意加的
        ...
        print('xV1_cube_equi shape:', xV1_cube_equi.shape)
        ...
        print('xV1_equi shape:', xV1_equi.shape)
        xV1_equi = xV1_equi*self.w + xV1_cube_equi*(1-self.w)
        ...
        sal = F.interpolate(sal, size=(320, 640), mode='bilinear', align_corners=True)  # 这一行完全是为了让模型跑通，看loss而刻意加的
